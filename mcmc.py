...
class MCMCSampleCollection(object):
    ...
    # 
    # At its core, this class is simply a convenient way to store 'sample tuples'. A sample tuple is
    # of the form:
    ...
    def init_var(self, var_name, var_value):
    ...
    def add_sample_by_index(self, var_ind, value, same_sample_as_prev=False):
        if not same_sample_as_prev: self.num_samples += 1
    ...
    def first_tuple_index_of_desired_sample_index(self, desired_sample_ind):
        if desired_sample_ind > self.num_samples - 1:
            sys.exit( 'Desired index %d is out of bounds with number of samples %d' \
                            % (desired_sample_ind, self.num_samples) )

        elif desired_sample_ind <= 10:
            ...
            while True:
                ...
                # Also note that test_ind < len(self.sample_tuples) as integer division rounds down.
    ...
    def full_posterior_samples(self, starting_sample_ind=0, ending_sample_ind=self.num_samples):
        ...
        for sample_ind, var_ind, value in self.sample_tuples:
            if current_sample_ind != sample_ind:
                ...
                if current_sample_ind >= ending_sample_ind: break
            current_sample[var_ind] = value
        return posterior_samples

    def marginal_posterior_samples_by_name(self, marg_var_names, starting_sample_ind=0, ending_sample_ind=self.num_samples):
        marg_var_inds = [self.var_index_given_name[name] for name in marg_var_names]
        self.marginal_posterior_samples_by_index(marg_var_inds, starting_sample_ind=0, ending_sample_ind=self.num_samples)

    def marginal_posterior_samples_by_index(self, marg_var_inds, starting_sample_ind=0, ending_sample_ind=self.num_samples):
